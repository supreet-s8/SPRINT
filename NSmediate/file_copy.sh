...
if [[ "$last" == "excstring" ]]
        then
                string=$5
                out=`find $l_dir -type f -mmin +2 -mmin -8 -exec basename {} \; |egrep -v $string`
                for i in `echo "$out"`
                do
                        if [ -d "$tree/$h" ]
                                then
...
if [[ "$last" == "all" ]]
		...
		out=`find $l_dir -type f -mmin +2 -mmin -8 -exec basename {} \; | egrep -w "$inp" | egrep "$in_string" | egrep -v "$ex_string"`
                for i in `echo "$out"`
                do	
                        if [ -d "$tree/$h" ]
                                then
...
if [[ "$last" == "ipstring" ]]
        then
                i=$5
                in_string=$6
		out=`find $l_dir -type f -mmin +2 -mmin -8 -exec basename {} \; |egrep  "$i" | egrep "$in_string"`
		#out=`find $l_dir -type f -mmin +2 -mmin -15 -exec basename {} \; |egrep  "10.32.8.6" | egrep "HTTP"`
                for i in `echo "$out"`
                do      
                        if [ -d "$tree/$h" ]
                                then
...
if [[ "$last" == "ipexcstring" ]]
        then
                i=$5
                in_string=$6
                out=`find $l_dir -type f -mmin +2 -mmin -8 -exec basename {} \; |egrep -w "$i" | egrep -v "$in_string"`
                for i in `echo "$out"`
                do
                        if [ -d "$tree/$h" ]
                                then
...
if [[ "$last" == "onlystring" ]]
        then
                ...
                in_string=$5
		ex_string=$6
		out=`find $l_dir -type f -mmin +2 -mmin -8 -exec basename {} \; | egrep "$in_string" | egrep -v "$ex_string"`
                for i in `echo "$out"`
                do
