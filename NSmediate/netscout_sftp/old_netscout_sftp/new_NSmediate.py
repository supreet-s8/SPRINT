...
class Server(object):
    ...
    def __init__(self, options):
        ...
        if not os.path.exists(self.retrydir):
            try:
                ...
                logging.debug("Creating retry file directory: " + self.retrydir)
            except:
                logging.error("Unable to create retry file directory: " + self.retrydir)
    ...
    def get_retryfiles(self):
        ...
        rtlsraw = pexpect.run('ls -lrt ' + self.retrydir) # Capture local ls -lrt (oldest first)
        if not rtlsraw:
            rtlsraw = dirslug
    ...
    def check_duptmpfile(self, f):
        ...
        if rfiles["tmp_" + f] and rfiles[f]:
            if rfiles[f] == rfiles["tmp_" + f]:
                sftp_rm("tmp_" + f)
                logging.info("Deleted duplicate file tmp_" + f)
                dup = True
        return dup
    ...
    def sftp_rename(self, f):
        ...
        elif check_duptmpfile(f):
            logging.info("COMPLETED: (removed duplicate)" + f + " at " + str(now)) # Log completion
        else:
            logging.error("Unknown error in rename of file " + f)
    ...
    def sftp_copy(self, f, ldir):
        ...
        if copy_success:				     # If True
            logging.debug("File " + lfile + " successfully copied to tmp_" + f) # Log success
            self.sftp_rename(f)				     # Attempt to rename
        else:						     # Otherwise:
            now = datetime.datetime.now()		     # Get a timestamp
            logging.error("Unsuccessful file transfer: " + lfile + " at " + str(now)) # Log failure
            try:
                if not ldir == self.retrydir:
                    shutil.copy2(lfile, self.retrydir)
                    logging.debug("Copied: " + f + " to " + self.retrydir) # Log copy action
                else:
                    logging.error("File " + f + " failed repeat upload attempt from " + self.retrydir)
                    logging.debug("File " + f + " will remain in " + self.retrydir)
            except:
                logging.error("Unable to copy " + f + " to " + self.retrydir)
    ...
    def process(self):
        ...
        self.do_sftp(dutydict, self.localdir)	# Do the actual sftp stuff
